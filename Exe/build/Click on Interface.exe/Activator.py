...
def mainMethod(list_of_courses, styleNo):
	...
	if len(list_of_courses) == 2:
		...
		ZaList = []
		urlExtract(list_of_courses, ZaList, path)
		coursesTbls = []
		for i in ZaList:
...
def twoCourses(list_of_courses):
	...
	if __name__ == "Activator":
		...
		with M.Manager() as manager:
			l = manager.list()
			m1 = M.Process(target= urlExtract, args=( c1, l, path))
			m2 = M.Process(target= urlExtract, args=( c2, l,path))
			m1.start()
			m2.start()
			m1.join()
			m2.join()
...
def threeCourses(list_of_courses):
	...
	if __name__ == "Activator":
		...
		with M.Manager() as manager:
			...
			m1 = M.Process(target= urlExtract, args=( c1, l, path))
			m2 = M.Process(target= urlExtract, args=( c2, l, path))
			m3 = M.Process(target= urlExtract, args=( c3, l, path))
			m1.start()
			m2.start()
			m3.start()
			m1.join()
